...
import { X, Calendar, Tag, CreditCard, AlignLeft, ArrowUpCircle, ArrowDownCircle, Repeat, User, Users, Wallet } from 'lucide-vue-next';
...
// Cores dinâmicas
const amountColor = computed(() => type.value === 'receita' ? 'text-[var(--color-success)]' : 'text-[var(--color-danger)]');
const btnColor = computed(() => type.value === 'receita' 
    ? 'bg-[var(--color-success)] hover:brightness-110 shadow-lg shadow-[var(--color-success)]/20' 
    : 'bg-[var(--color-danger)] hover:brightness-110 shadow-lg shadow-[var(--color-danger)]/20');
...
const fetchOptions = async () => {
  try {
    const [catsRes, cardsRes] = await Promise.all([
       api.get('/options/categories').catch(() => ({ data: [] })),
       api.get('/options/credit-cards').catch(() => ({ data: [] }))
    ]);
...
watch(() => props.transactionToEdit, (tx) => {
  if (tx) {
    ...
    cardId.value = tx.credit_card_id || '';
...
const handleSubmit = async () => {
  ...
  const finalInstallments = (isCredit && installments.value) ? parseInt(installments.value) : 1;
  ...
  try {
    if (isEditing.value) await api.put(`/transactions/${props.transactionToEdit.id}`, payload);
...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-[360px] bg-[var(--bg-surface)] rounded-3xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden border border-[var(--border)] animate-in zoom-in-95 duration-200">
      
      <div class="px-5 pt-4 pb-2 flex justify-between items-center bg-[var(--bg-surface)] z-10 shrink-0">
        <h2 class="text-xs font-bold text-[var(--text-main)] uppercase tracking-widest opacity-70">{{ isEditing ? 'Editar' : 'Nova Transação' }}</h2>
        <button @click="$emit('close')" class="p-1.5 -mr-2 rounded-full hover:bg-[var(--bg-app)] text-[var(--text-muted)] transition-colors">
            <X :size="20" />
        </button>
      </div>

      <div class="flex-1 overflow-y-auto custom-scroll px-5 pt-0 pb-2 space-y-4">
        
        <div class="flex p-1 bg-[var(--bg-app)] rounded-xl border border-[var(--border)] relative shrink-0 mt-2">
            <button @click="type = 'despesa'" class="flex-1 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-all relative z-10"
            ...
            <button @click="type = 'receita'" class="flex-1 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-all relative z-10"
        ...
        <div class="text-center py-2">
            <div class="flex items-center justify-center gap-1">
                <span class="text-xl font-bold opacity-50 mb-1" :class="amountColor">R$</span>
                <input v-model="displayAmount" type="tel" placeholder="0,00" 
                       class="bg-transparent text-4xl font-bold text-center w-full focus:outline-none placeholder-[var(--text-muted)]/20 font-numeric py-0" 
                       :class="amountColor"
                       autofocus />
            </div>
        </div>
        ...
        <div class="space-y-3">
            <div class="bg-[var(--bg-app)]/50 rounded-xl px-3 py-3 flex items-center gap-3 border border-[var(--border)] focus-within:border-[var(--color-primary)] focus-within:bg-[var(--bg-surface)] transition-all">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="bg-[var(--bg-app)]/50 rounded-xl px-3 py-3 flex items-center gap-2 border border-[var(--border)] focus-within:border-[var(--color-primary)] focus-within:bg-[var(--bg-surface)] transition-all">
                    <Tag :size="16" class="text-[var(--text-muted)]" />
                    <select v-model="categoryId" class="w-full bg-transparent text-sm font-medium text-[var(--text-main)] focus:outline-none dark-scheme-select">
                ...
                <div class="bg-[var(--bg-app)]/50 rounded-xl px-3 py-3 flex items-center gap-2 border border-[var(--border)] focus-within:border-[var(--color-primary)] focus-within:bg-[var(--bg-surface)] transition-all">
                    ...
                    <input v-model="date" type="date" class="w-full bg-transparent text-sm font-medium text-[var(--text-main)] focus:outline-none dark-scheme-select" />
                </div>
            </div>
        </div>

        <div v-if="type === 'despesa' && enableDebts" class="space-y-1.5 animate-in fade-in duration-300 pt-1">
            <p class="text-[10px] text-[var(--text-muted)] font-bold uppercase px-1">Para quem é a compra?</p>
            <div class="flex gap-2">
                <button @click="debtorType = 'eu'" class="flex-1 py-2.5 rounded-xl border flex items-center justify-center gap-2 text-xs font-bold transition-all"
                        :class="debtorType === 'eu' 
                            ? 'border-[var(--color-primary)] bg-[var(--color-primary-light)] text-[var(--color-primary)]' 
                            : 'border-[var(--border)] bg-[var(--bg-app)]/30 text-[var(--text-muted)]'">
                    <User :size="14" /> Para mim
                </button>
                <button @click="debtorType = 'outro'" class="flex-1 py-2.5 rounded-xl border flex items-center justify-center gap-2 text-xs font-bold transition-all"
            ...
            <div v-if="debtorType === 'outro'" class="animate-in slide-in-from-top-1 duration-200">
                <div class="bg-[var(--bg-app)]/50 rounded-xl px-3 py-3 flex items-center gap-2 border border-[var(--color-warning)]/30">
                    ...
                    <input v-model="debtorName" type="text" placeholder="Nome da pessoa" class="flex-1 bg-transparent text-sm font-medium text-[var(--text-main)] placeholder-[var(--text-muted)] focus:outline-none" />
                </div>
            </div>
        </div>

        <div class="space-y-1.5 pt-1">
            <p class="text-[10px] text-[var(--text-muted)] font-bold uppercase px-1">Forma de Pagamento</p>
            ...
            <div v-if="paymentMethod === 'credito'" class="space-y-2.5 animate-in slide-in-from-top-1 duration-200 pt-1">
                <div class="bg-[var(--bg-app)]/50 rounded-xl px-3 py-3 flex items-center gap-2 border border-[var(--border)]">
                    <CreditCard :size="16" class="text-[var(--text-muted)]" />
                    <select v-model="cardId" class="w-full bg-transparent text-sm font-medium text-[var(--text-main)] focus:outline-none dark-scheme-select">
                ...
                <div class="bg-[var(--bg-app)]/50 rounded-xl px-3 py-3 flex items-center gap-2 border border-[var(--border)]">
                    ...
                    <input v-model="installments" type="number" min="1" max="24" class="flex-1 bg-transparent text-sm font-medium text-[var(--text-main)] focus:outline-none" />
                    <span class="text-xs text-[var(--text-muted)]" v-if="amount && installments > 1">
        ...
        <div class="rounded-xl border border-[var(--border)] bg-[var(--bg-app)]/30 overflow-hidden transition-all duration-300" 
            ...
            <div class="flex items-center justify-between p-3 cursor-pointer" @click="isFixed = !isFixed">
                <div class="flex items-center gap-2">
                    <div class="p-1 rounded bg-[var(--bg-surface)] text-[var(--text-muted)] border border-[var(--border)]">
                        <Repeat :size="12" />
                    </div>
                    <div>
                        <p class="text-xs font-bold text-[var(--text-main)]">Recorrência (Fixa)</p>
                    </div>
                </div>
                <div class="relative inline-block w-8 h-4 transition duration-200 ease-in-out rounded-full"
                     :class="isFixed ? 'bg-[var(--color-primary)]' : 'bg-[var(--border)]'">
                    <span class="absolute left-1 top-1 w-2 h-2 bg-white rounded-full transition-transform duration-200 ease-in-out shadow-sm"
                          :class="isFixed ? 'translate-x-4' : 'translate-x-0'"></span>
                </div>
            </div>

            <div class="grid transition-[grid-template-rows] duration-300 ease-out"
                 :class="isFixed ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'">
                <div class="overflow-hidden">
                    <div class="px-3 pb-3">
                        <div class="bg-[var(--bg-surface)] rounded-xl px-3 py-2 flex items-center gap-2 border border-[var(--border)]">
                            ...
                            <select v-model="recurrencePeriod" class="flex-1 bg-transparent text-xs font-bold text-[var(--text-main)] focus:outline-none dark-scheme-select text-right">
      ...
      <div class="p-5 pt-2 bg-[var(--bg-surface)] border-t border-[var(--border)] z-20 shrink-0">
        <button @click="handleSubmit" :disabled="isLoading" 
                class="w-full py-3.5 rounded-xl font-bold text-sm text-white transition-all active:scale-95 flex items-center justify-center gap-2 shadow-md"
                :class="btnColor">
