...
# Importa os modelos e enums do seu tables.py
...
class DashboardService:
    ...
    def get_summary(self, user_id: int, month: int, year: int):
        # Define datas de inicio e fim do mês
        start_date = date(year, month, 1)
        last_day = calendar.monthrange(year, month)[1]
        end_date = date(year, month, last_day)

        # 1. Saldo Atual (Soma o saldo de todas as contas do usuário)
        ...
        # O SQLAlchemy converte TransactionType.INCOME para "income" automaticamente
        ...
        # 3. Pendências (Runway) - Contas PENDENTES até o fim do mês
    ...
    def get_category_breakdown(self, user_id: int, month: int, year: int):
        ...
        return [
            {"name": r.name, "value": float(r.total), "color": r.color} 
            for r in results
        ]

    def get_upcoming_transactions(self, user_id: int, days: int = 7):
        """Busca contas a pagar nos próximos X dias"""
        today = date.today()
